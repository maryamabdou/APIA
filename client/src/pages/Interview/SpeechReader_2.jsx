...
const SpeechReader = () => {
  ...
  const handleSpeakClick = (text) => {
    ...
    const audioElement = generateAudioFromSpeech(text)
    sendAudioToFlask(audioElement)
  };
...
// const sendAudioToFlask = (audioElement) => {
//     const audioBlob = new Blob(audioElement, { type: 'audio/wav' });
...
const sendAudioToFlask = async (audioElement) => {
  // Get browser-compatible media stream
  try {
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true });

    // Create Blob from MediaStream
